...
function nextId() {
  messageIdCounter += 1;
  return `dataentry-m${messageIdCounter}`;
}

export function DataEntryAIAssistantPanel() {
  ...
  const firstName = DATAENTRY_PROFILE.name.split(' ')[0];
        ...
        <>
          ...
          <div className="assistant-conversation">
            ...
            <div className="assistant-conversation__composer">
              <DataEntryComposer draft={draft} setDraft={setDraft} onSend={() => sendPrompt(draft)} />
            </div>
          </div>
        </>
      ) : (
...
function DataEntryComposer({ draft, setDraft, onSend }: ComposerProps) {
...
function DataEntrySuggestionIcon() {
  return (
